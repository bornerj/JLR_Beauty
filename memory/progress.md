...
| Cadastros/Sistema nativos no Admin V2 (PLAN-0026) | planning | 2026-08-16 | Plano escrito, aguardando aprovação do usuário pra iniciar execução. `DECISION-014` (ACTIVE) substitui a regra #5 da `DECISION-013` — autorização explícita do usuário. RAG confirmou: 100% do backend CRUD já existe (`catalog.ts`/`users.ts`/`schedule.ts`/`subscriptions.ts`/`admin.ts`), nenhum endpoint novo previsto. 14 ondas (1 tela/entidade por onda, "Pessoas" desmembrada em Clientes/Profissionais/Usuários por modularidade), sequenciadas por complexidade real: 7 ondas tier P (quase reskin — Planos/Entrega/Branding/Cupons/Textos/Seções/Galeria), 3 tier M (Serviços/WhatsApp/Testes — têm `behavior.ts` imperativo a reescrever), 4 tier G por último (Produtos, Clientes, Profissionais, Usuários — mais relações/complexidade). "Segurança"/"Infra" do hub Sistema ficam fora — sem tela legada equivalente. |
