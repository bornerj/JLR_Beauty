...
| Admin V2 — Inteligência (PLAN-0023) | in-progress | 2026-08-15 | Continuação do `PLAN-0022`, mesma branch. **Ondas 1-4 concluídas e validadas (E2E real + visual)**: Radar Executivo (RETROFIT-011, `/admin-v2/radar`, 8 módulos agregados); Gargalos (RETROFIT-012, `/admin-v2/gargalos`, ranking por impacto R$ honesto); "Onde está o dinheiro?" (RETROFIT-013, `/admin-v2/dinheiro`, cascata Receita→Custo Direto→Margem Bruta + 6 decomposições); Comparador de Unidades (RETROFIT-014, `/admin-v2/comparador`, 5 dimensões × unidade + achado de maior diferença, R$ estimado só para Ocupação). `test:intelligence` com 108 testes; `npm run test` (api) 136/136 PASS. **Commitado, pushado e incluído no PR #1** (mesmo commit `c00a21d` do PLAN-0022). Próximo passo: usuário decide entre RETROFIT-017/018/019 ou aguardar revisão/merge do PR #1. |
