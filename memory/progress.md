...
| Cadastros/Sistema nativos no Admin V2 (PLAN-0026) | in-progress | 2026-08-16 | `DECISION-014` (ACTIVE) substitui a regra #5 da `DECISION-013`. 14 ondas planejadas, sequenciadas por complexidade real. **Ondas 1-8 concluídas e validadas de verdade** (E2E real + visual real) — **tier P inteiro fechado** (Planos/Entrega/Branding/Cupons/Textos das Páginas/Seções Telas/Galeria de Mídias) **+ Onda 8 (Serviços), primeira do tier M**: primeira reescrita de um `behavior.ts` imperativo (416 linhas) como React declarativo, com `CategoryStatusManagerModal` genérico (novo, reusável na Onda 11/Produtos) pra categorias/status. 3 bugs reais achados e corrigidos no tier P: `ERR-0050` (schema Zod de cupom), `ERR-0051` (CSS estático — mesma causa raiz do `ERR-0049`/`ERR-0040`, regenerado + nota de processo), `ERR-0052` (z-index de modal aninhado herdado do legado). Achado de contrato recorrente (Ondas 5 e 7): telas de config-map grande sempre substituem o mapa inteiro no `PUT`, nunca fazem merge incremental. `DeleteConfirmModal` generalizado (`tone`/`confirmLabel`). `HubCard` ganhou o modo `native`; breadcrumb das sub-telas de Cadastros e Sistema refatorado pra tabela de lookup. Usuário autorizou execução autônoma do resto do plano: commit sem perguntar por onda, push só no final, próxima onda inicia sem aguardar aprovação. **Ondas 9 (WhatsApp/Integrações), 10 (Testes) e 11 (Produtos) também concluídas — tier P/M inteiramente fechados + Onda 11, a mais pesada do plano inteiro**. Onda 9 reusa `/concierge/sessions` (filtro server-side) + `/api/settings/:key` genérico. Onda 10 (fecha o tier M) teve decisão de escopo: 13 smoke-checks de API portados, checagens de DOM do shell legado descartadas (nunca existiriam na árvore do Admin V2). Onda 11 (Produtos, tier G) entregou CRUD completo + estoque multi-unidade real (movimentar/histórico/cross-unit), e teve um **achado de RAG corrigido por E2E real**: a varredura inicial (grep por padrão ORM) concluiu errado que `Product.stock` nunca era atualizado — só descobrindo o erro testando de verdade (o campo É mantido via `$executeRaw`, que o grep não pegou); tabela corrigida pra mostrar "Estoque total" real. Achado de backend confirmado e documentado (`ERR-0053`, fora de escopo): produtos com histórico de estoque nunca podem ser excluídos (500, falta `onDelete: Cascade`). `CategoryStatusManagerModal` generalizado (`entity: "service" \| "product"`) pra servir as duas entidades sem duplicar. **Onda 12 (Clientes) também concluída** — primeira do desmembramento de "Pessoas" (`DECISION-014` regra #3): card único do hub virou 3 (Clientes nativo, Profissionais/Usuários ainda legado). `Customer` não tem `DELETE` no backend — tela nativa também não tem botão de excluir (não fabricado). Achado real (`ERR-0054`): nomear a sub-rota `cadastros/clientes` colidiu com o mundo de analytics "Clientes" já existente (`.includes("/clientes")` casava os dois) — corrigido ancorando a checagem ao path completo; nota de processo pra checar colisão de slug em ondas futuras. Faltam Ondas 13-14 (Profissionais/Usuários, fecham o desmembramento de "Pessoas" e o plano inteiro). |
