...
| Estoque Multi-Unidade + Vendas + BI (PLAN-0020) | in-progress | 2026-07-07 | EXECUTADO ponta a ponta — ledger auditável, reserva TTL 20min+sweeper, RBAC unidade fail-closed, venda balcão (PAGO, canal+vendedor+unidade, total server-side), Stripe reserva→confirma, Loja Online (unit 3), KPIs CMV/margem/tops, admin produtos por unidade, venda manual, BI dashboard, SPA Esgotado. Validado e2e via API real; dados de teste limpos. Checklist do plano (ondas 0-7, critérios de aceitação, travas S1-S9) fechado; rodada 1 de validação visual do usuário concluída (4 ajustes); testes automatizados do ledger/reserva/RBAC implementados (23/23 PASS). Falta: confirmação final do usuário + pentest S10 + commit/push → DONE. |
