...
| Cadastros/Sistema nativos no Admin V2 (PLAN-0026) | in-progress | 2026-08-16 | `DECISION-014` (ACTIVE) substitui a regra #5 da `DECISION-013`. 14 ondas planejadas, sequenciadas por complexidade real. **Ondas 1-7 concluídas e validadas de verdade** (E2E real + visual real) — **tier P inteiro fechado**: Planos, Entrega, Branding, Cupons, Textos das Páginas, Seções Telas e Galeria de Mídias (`/admin-v2/sistema/galeria-midias`, 78 slots, editor em modal, fallback em cascata preservado). 3 bugs reais achados e corrigidos ao longo do tier P: `ERR-0050` (schema Zod de criação de cupom), `ERR-0051` (CSS estático não pegava classes novas do switch de Seções — mesma causa raiz do `ERR-0049`/`ERR-0040`, regenerado + nota de processo pra regenerar proativamente daqui pra frente), `ERR-0052` (z-index herdado do legado deixava o modal de confirmação de fechar inacessível atrás do editor de mídia). Achado de contrato recorrente (Ondas 5 e 7): telas de config-map grande (`page-texts`, `media-slots`) sempre substituem o mapa inteiro no `PUT`, nunca fazem merge incremental — a tela sempre carrega e reenvia o estado completo. `DeleteConfirmModal` generalizado (`tone`/`confirmLabel`). `HubCard` ganhou o modo `native`; breadcrumb das sub-telas de Cadastros e Sistema refatorado pra tabela de lookup. Usuário autorizou execução autônoma do resto do plano: commit sem perguntar por onda, push só no final, próxima onda inicia sem aguardar aprovação. Faltam Ondas 8-14, todas tier M/G (Serviços/WhatsApp/Testes — `behavior.ts` imperativo a reescrever; Produtos/Clientes/Profissionais/Usuários — as mais pesadas do plano). |
