...
| Admin V2 — Inteligência (PLAN-0023) | in-progress | 2026-08-15 | Continuação do `PLAN-0022`, mesma branch. **Ondas 1-5 concluídas e validadas (E2E real + visual)**: Radar Executivo (RETROFIT-011, `/admin-v2/radar`, 8 módulos agregados); Gargalos (RETROFIT-012, `/admin-v2/gargalos`, ranking por impacto R$ honesto); "Onde está o dinheiro?" (RETROFIT-013, `/admin-v2/dinheiro`, cascata Receita→Custo Direto→Margem Bruta + 6 decomposições); Comparador de Unidades (RETROFIT-014, `/admin-v2/comparador`, 5 dimensões × unidade + achado de maior diferença, R$ estimado só para Ocupação); Health Score evolução (RETROFIT-017, narrativa em prosa no Diagnóstico da Unidade + correção do botão "Comparar unidade" que estava desatualizado). `test:intelligence` com 121 testes; `npm run test` (api) 149/149 PASS. **Ondas 1-4 commitadas, pushadas e no PR #1**; **Onda 5 (RETROFIT-017) ainda não commitada** — feita após o PR #1 ser aberto. Próximo passo: usuário decide entre RETROFIT-018/019, commit/push da Onda 5, ou revisão/merge do PR #1. |
