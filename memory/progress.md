...
| **Cadastros/Sistema nativos no Admin V2 (futuro `PLAN-0026`)** | planned | 2026-08-15 | Desmembrado do `PLAN-0025` (item 2 do pedido do usuário) — contraria `DECISION-013` regra #5 hoje ativa (adapter/link pro legado, sem reescrita estética). Usuário decidiu (gate socrático): plano separado. **Pré-requisito antes de criar o `PLAN-0026`**: atualizar `DECISION-013` regra #5 formalmente (novo `DECISION-XXX` ou emenda) + priorizar a ordem das ~10 telas legadas (Produtos/Serviços/Planos/Cupons/Entrega/Pessoas 3 sub-abas/Branding/Textos/Seções/Galeria/WhatsApp/Testes). Nada de execução fabricado ainda. |
