...
| Cadastros/Sistema nativos no Admin V2 (PLAN-0026) | in-progress | 2026-08-16 | `DECISION-014` (ACTIVE) substitui a regra #5 da `DECISION-013`. 14 ondas planejadas, sequenciadas por complexidade real. **Ondas 1-3 concluídas e validadas de verdade** (E2E real + visual real): Planos (`/admin-v2/cadastros/planos`, CRUD completo), Entrega (`/admin-v2/cadastros/entrega`, config-form) e Branding (`/admin-v2/sistema/branding`, config-form + upload de logo + histórico local, rota dedicada `/admin/branding` — não o `/settings/:key` genérico). `HubCard` ganhou o modo `native`; breadcrumb das sub-telas de Cadastros e agora também Sistema refatorado pra tabela de lookup (não infla o arquivo a cada onda). Usuário autorizou execução autônoma do resto do plano: commit sem perguntar por onda, push só no final, próxima onda inicia sem aguardar aprovação. Faltam Ondas 4-14 (Cupons/Textos/Seções/Galeria — tier P; Serviços/WhatsApp/Testes — tier M; Produtos/Clientes/Profissionais/Usuários — tier G). |
