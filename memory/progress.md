...
| Cadastros/Sistema nativos no Admin V2 (PLAN-0026) | in-progress | 2026-08-16 | `DECISION-014` (ACTIVE) substitui a regra #5 da `DECISION-013`. 14 ondas planejadas, sequenciadas por complexidade real. **Ondas 1-6 concluídas e validadas de verdade** (E2E real + visual real): Planos, Entrega, Branding, Cupons, Textos das Páginas e Seções Telas (`/admin-v2/sistema/secoes`, 32 chaves, só MASTER edita — gate preservado). Onda 4 pegou um bug real de backend (`ERR-0050`, schema Zod). Onda 5 confirmou achado de contrato crítico: `savePublicPageTexts` substitui o mapa inteiro, não faz merge incremental. Onda 6 pegou outro bug real de CSS (`ERR-0051`, mesma causa raiz do `ERR-0049`/`ERR-0040`): `tailwind.generated.css` é snapshot estático, classes novas de UI (switch customizado) somem silenciosamente até regenerar o arquivo — regenerado, e nota de processo criada pra próximas ondas regenerarem proativamente quando introduzirem padrão visual novo. `DeleteConfirmModal` generalizado (`tone`/`confirmLabel`). `HubCard` ganhou o modo `native`; breadcrumb das sub-telas de Cadastros e Sistema refatorado pra tabela de lookup. Usuário autorizou execução autônoma do resto do plano: commit sem perguntar por onda, push só no final, próxima onda inicia sem aguardar aprovação. Faltam Ondas 7-14 (Galeria — tier P; Serviços/WhatsApp/Testes — tier M; Produtos/Clientes/Profissionais/Usuários — tier G). |
