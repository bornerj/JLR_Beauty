...
| Cadastros/Sistema nativos no Admin V2 (PLAN-0026) | in-progress | 2026-08-16 | `DECISION-014` (ACTIVE) substitui a regra #5 da `DECISION-013`. 14 ondas planejadas (1 tela/entidade por onda), sequenciadas por complexidade real. **Onda 1 (Planos) concluída e validada de verdade** (E2E real + visual real) — primeira tela nativa de Cadastros, `/admin-v2/cadastros/planos`, CRUD completo reusando `/api/memberships` sem endpoint novo. `HubCard` ganhou o modo `native` (legenda muda, card do hub vira link interno). Achado da Onda 1: a interação do legado às vezes vive centralizada em `admin-core/behavior.ts` (não num `behavior.ts` próprio do módulo) — atenção nas próximas ondas ao estimar complexidade só por linhas do módulo. Faltam Ondas 2-14 (Entrega/Branding/Cupons/Textos/Seções/Galeria — tier P; Serviços/WhatsApp/Testes — tier M; Produtos/Clientes/Profissionais/Usuários — tier G). Sem commit/push (aguardando autorização). |
