...
export const HomeCtaSection = (): ReactElement => {
  const eyebrow  = usePageText("home.cta.eyebrow");
  const title    = usePageText("home.cta.title");
  const subtitle = usePageText("home.cta.subtitle");
  const button   = usePageText("home.cta.button");
        ...
        <div className="w-full rounded-3xl border border-gold-accent/60 bg-[#0f2418] dark:bg-[#0b1811] overflow-hidden relative">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between p-12 md:p-16 gap-6">
                <div className="max-w-2xl">
                    <p className="text-gold text-xs uppercase tracking-[0.35em] mb-3"><RichText value={eyebrow} /></p>
                    <h2 className="text-3xl md:text-4xl display-hero text-gold mb-4"><RichText value={title} /></h2>
                    <p className="text-gold/80 text-lg font-light"><RichText value={subtitle} /></p>
                </div>
